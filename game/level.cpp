...
void level_init(GameState& g) {
    ...
    auto addGhost = [&](uint16_t color, int id, int col, int row) {
        g.ghosts.push_back(Ghost(color, id, col, row));
    ...
    addGhost(COLOR_RED,       0, gcol,     grow);
    addGhost(COLOR_LIGHTBLUE, 1, gcol - 1, grow);
    addGhost(COLOR_PINK,      2, gcol + 1, grow);
    addGhost(COLOR_ORANGE,    3, gcol,     grow + 1);
}
